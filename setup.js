document.querySelector('#setup-form').addEventListener('submit', event => {
  ...
  const config = `window.DV_CONFIG = Object.freeze(${JSON.stringify({companyName:'DespedidaVerse Studio',contactEmail:d.email,siteUrl:d.siteUrl,appsScriptUrl:d.appsScriptUrl,tallyFormUrl:d.tallyFormUrl,whatsappNumber:d.whatsapp,paymentLinks:{esencial:d.paymentEssential,juego:d.paymentGame,universo:d.paymentUniverse,live:''},commerceApiUrl:d.commerceApiUrl,commerce:{currency:'EUR',vatIncluded:true,paymentProvider:'stripe',checkoutPolicy:'accepted_proposal_only',productionRequires:'server_verified_payment',allowedStates:['pending','paid','failed','refunded']},pilotEnabled:d.pilotEnabled==='on',clientAreaEnabled:true,onboardingEnabled:true,directSubmitEnabled:true,siteVersion:'v24-commerce-pilot'}, null, 2)});\n`;
